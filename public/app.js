...
$("#promptForm").addEventListener("submit", (event) => { event.preventDefault(); const input = $("#prompt"); const text = input.value.trim(); if (!text) return; input.value = ""; sendPrompt(text).catch((error) => { state.taskState = "FAILED"; setStatus("FAILED", error.message); state.messages.push({ role: "assistant", text: `无法提交任务：${error.message}` }); renderMessages(); }); });
$("#prompt").addEventListener("keydown", (event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); $("#promptForm").requestSubmit(); } });
